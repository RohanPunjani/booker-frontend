...
export default function BookedFlights() {
  ...
  useEffect(() => {
    const fetchBookedFlightsData = async () => {
      ...
      setUserBookedFlights(data.data);
    };
        ...
        <div className="container">
          ...
          <div className="row">
            <div className="col-md-12 mb-4">
              ...
              <h1 className="display-4 pb-4"> Booked flights:</h1>
              {userBookedFlights.map((booking, idx) => {
                if (booking.bookingStatus === "canceled") return null;
                return <Ticket booking={booking} key={idx} />;
              })}
            </div>
          </div>
        </div>
